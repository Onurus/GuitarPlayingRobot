package usta.onur.ceng599.util;
...
/**
 * This {@link AWTEventListener} tries to work around <a
 * href="http://bugs.sun.com/view_bug.do?bug_id=4153069">a 12 yo bug</a> in the
 * Linux KeyEvent handling for keyboard repeat. Linux apparently implements
 * repeating keypresses by repeating both the {@link KeyEvent#KEY_PRESSED}
 * <b>and</b> {@link KeyEvent#KEY_RELEASED}, while on Windows, one only gets
 * repeating PRESSES, and then a final RELEASE when the key is released. The
 * Windows way is obviously much more useful, as one then can easily distinguish
 * between a user holding a key pressed, and a user hammering away on the key.
 ...
 * but it is simpler to invoke {@link #install() install(new instance)}: <br>
 * 
 * <pre>
 * *
 ...
 * install it again - or else you'll end up with multiple instances, which isn't
 * too hot. <b>Notice:</b> Read up on the {@link Reposted} interface if you have
 * other AWTEventListeners that resends KeyEvents (as this one does) - or else
 * we'll get the event back. <br>
 * <h3>Mode of operation</h3> The class makes use of the fact that the
 * subsequent PRESSED event comes right after the RELEASED event - one thus have
 ...
 * that denotes that the key actually was released. Inspired by <a
 * href="http://www.arco.in-berlin.de/keyevent.html"
 * >http://www.arco.in-berlin.de/keyevent.html</a>
 *
...
public class RepeatingReleasedEventsFixer implements AWTEventListener
{
	...
	public void eventDispatched(AWTEvent event)
	{
		...
		if(keyEvent.getID() == KeyEvent.KEY_RELEASED)
		{
			 ...
			 * handles if the Timer thread actually fires the action due to lags,
			 * by cancelling the action itself upon the PRESSED.
			 */
	...
	/**
	 * The ActionListener that posts the RELEASED {@link RepostedKeyEvent} if the
	 * {@link Timer} times out (and hence the repeat-action was over).
	 */
	private class ReleasedAction implements ActionListener
	{
		...
		public void actionPerformed(@SuppressWarnings("unused") ActionEvent e)
		{
			...
			// to be posted onto EDT by TimerQueue,
			...
			KeyEvent newEvent = new RepostedKeyEvent((Component) _originalKeyEvent.getSource(), _originalKeyEvent.getID(),
					_originalKeyEvent.getWhen(), _originalKeyEvent.getModifiers(), _originalKeyEvent.getKeyCode(),
					_originalKeyEvent.getKeyChar(), _originalKeyEvent.getKeyLocation());
	...
	{
		public RepostedKeyEvent(@SuppressWarnings("hiding") Component source, @SuppressWarnings("hiding") int id, long when,
